...
bool MopekaProCheck::parse_device(const esp32_ble_tracker::ESPBTDevice &device) {
  ...
  if (quality_value < this->min_signal_quality_to_report_) {
...
/**
 * @brief Set the min signal quality to report value.
 *
 * @param min - lowest read quality value to report for distance and level sensors
 *              If outside of the range of SensorReadQuality enum a default of QUALITY_MED
 *              will be used.
 */
void MopekaProCheck::set_min_signal_quality_to_report(int min) {
  SensorReadQuality value = static_cast<SensorReadQuality>(min);
  if ((value < QUALITY_NONE) || (value > QUALITY_HIGH)) {
    ESP_LOGE(TAG, "Requested min_sensor_quality_to_report (%d) is outside of valid range", min);
    return;
  }
  ESP_LOGD(TAG, "Setting min_sensor_quality_to_report as %d", static_cast<int>(value));
  this->min_signal_quality_to_report_ = value;
}
