...
static const char *const TAG = "motor.hbridge";

void HBridgeMotor::set_hbridge_levels_(float a_level, float b_level) {
...
void HBridgeMotor::set_hbridge_levels_(float a_level, float b_level, float enable) {
  this->pin_a_->set_level(a_level);
  this->pin_b_->set_level(b_level);
...
motor::motorCall HBridgeMotor::brake() {
...
void HBridgeMotor::setup() {
  ...
  // Construct traits
  this->traits_ = motor::MotorTraits(this->oscillating_ != nullptr, true, true, this->speed_count_);
  this->traits_.set_supported_preset_modes(this->preset_modes_);
}

void HBridgeMotor::dump_config() {
  LOG_MOTOR("", "H-Bridge Motor", this);
...
void HBridgeMotor::control(const motor::MotorCall &call) {
...
void HBridgeMotor::write_state_() {
  ...
  } else if (this->direction == motor::MotorDirection::FORWARD) {
    if (this->decay_mode_ == DECAY_MODE_SLOW) {
  ...
  } else {  // motor::Motor_DIRECTION_REVERSE
    if (this->decay_mode_ == DECAY_MODE_SLOW) {
